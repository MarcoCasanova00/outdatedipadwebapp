...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/todos":
            ...
            todos[id] = {"text": data["text"], "done": False}
            self.send_json({"todos": todos})
    ...
    <script>
        ...
        async function load() {
            ...
            document.getElementById('list').innerHTML = 
                Object.entries(todos.todos).map(([id,t]) => 
...
print("🚀 Server iPad - NO dependencies")
...
chmod +x ipad-server.py
python3 ipad-server.py
